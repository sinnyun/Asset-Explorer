...
本设计保留 SQLite 作为桌面端本地索引和用户数据存储，不做高风险的数据库整体替换；核心改造是将系统从“全量快照驱动”转为“数据库查询驱动 + 流式索引 + 有界任务 + 增量失效通知”。
...
- 用户字段：可以暂时保留，但文件扫描 UPSERT 禁止写入；最终建议迁移到 `asset_user_state`。
...
| 风险 | 缓解措施 |
|---|---|
| 大规模重构期间新旧状态并存 | 以查询 API 为切口逐视图迁移，保留短期适配层和功能开关 |
...
| 迁移期间用户数据损坏 | schema 版本、在线备份、字段所有权测试、可回滚迁移 |
...
1. 保留 SQLite，先修正连接、查询和任务模型；不以替换数据库掩盖全量架构问题。
