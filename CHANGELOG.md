...
## V2 clean-slate scalability refactor (2026-09-10)

- 使用独立 `assethub-v2.db`，不迁移或删除旧数据。
- 分离可重建文件事实与评分、收藏、颜色、标签、集合等用户状态。
- 增加 4 连接读池、256 容量单写入队列、SQLite WAL 与稳定游标查询。
- 启动移除全资产校验、周期深度对账和窗口回焦扫描。
- 索引改为 200 条流式批次、2 个固定 worker、32 容量任务队列及安全扫描代次。
- notify 改为 8192 容量事件队列、路径合并和目标子树更新。
- 主网格/列表与双分列改为独立分页和可视窗口渲染。
- 缩略图增加版本缓存键、2 worker、512 准入、100MP 解码前限制及 256 项前端 LRU。
- 删除通用整文件 Base64 预览，桌面预览改用 Tauri 资源 URL。
- 补齐 Web `/api/v2` 分页契约、PostgreSQL 唯一约束/索引及评分收藏持久化。
- 增加聚合性能诊断和带安全路径检查的规模数据生成器。
- 运行时移除旧全工作区加载、同步扫描、全量聚合、自动校验和全根对账命令。
